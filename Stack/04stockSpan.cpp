...
// 5 is greater than equal to 4 and 5 and smaller than 10 so the span is 2, and so on.
// Hence the output will be [1, 1, 2, 4, 5, 1].

// We need a stack for keeping track of the last greater element
...
void calculateSpan(vector<int> &arr)
{
    ...
    for (int i = 0; i < arr.size(); i++)
    {
        ...
        if (arr[prevMax] < arr[i])
        {
            prevMax = i;
            stk.push(prevMax);
        }

        ans.push_back(prevMax - i);
    }
